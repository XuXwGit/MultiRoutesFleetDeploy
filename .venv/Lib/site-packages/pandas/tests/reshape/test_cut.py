...
def test_datetime_bin(conv):
    ...
    binput_data = ["2012-12-12", "2012-12-14", "2012-12-16"]

    expected = Series(
        IntervalIndex(
            [
                Interval(Timestamp(binput_data[0]), Timestamp(binput_data[1])),
                Interval(Timestamp(binput_data[1]), Timestamp(binput_data[2])),
            ]
        )
    ).astype(CDT(ordered=True))

    bins = [conv(v) for v in binput_data]

...
class BendersLazyConsCallback:
    ...
    def set_models(self, determine_model: DetermineModel, dual_sub_problem: DualSubProblem):
        """
        设置模型
        
        Args:
            determine_model: 主问题模型
            dual_sub_problem: 子问题模型
        """
    ...
    def callback(self):
        ...
        try:
            ...
            # 检查子问题是否可行
            if self.dual_sub_problem.solve_status:
                # 生成Benders割
    ...
    def _generate_benders_cut(self, x_var: Dict, y_var: Dict, z_var: Dict, g_var: Dict):
        ...
        """
        # 获取子问题解
        v_var_value = self.dual_sub_problem.v_var_value
        u_value = self.dual_sub_problem.u_value
        
        # 构建割平面
        cut = self.determine_model.model_str.add_constraint(
            self.determine_model.theta >= 
            sum(v_var_value[r] * (sum(x_var[v, r] for v in self.input_data.vessels) - 1) 
        ...
        # 更新割平面计数
        self.cut_count += 1
        logger.info(f"Generated Benders cut {self.cut_count}")

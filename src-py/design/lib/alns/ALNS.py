...
class ALNS:
    ...
    def __init__(self, rng: rnd.Generator = rnd.default_rng(), optimization_direction: str = "minimize"):
        """
        Parameters
        ----------
        rng
        ...
        if optimization_direction not in ["minimize", "maximize"]:
            raise ValueError("optimization_direction must be either 'minimize' or 'maximize'")
        
        ALNS.optimization_direction = optimization_direction
    ...
    def iterate(
        ...
        **kwargs
        ...
        for iteration in tqdm(range(max_iterations), desc="Processing", ncols=100, bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt}| '):
            ...
            if (iteration + 1) % output_interval == 0:
                elapsed_time = stats.total_runtime
                # print()
                logging.debug(f"Iteration {iteration + 1}/{max_iterations} | "
                    f"Current: {curr.objective():.2f} |"
                    f"Best Obj: {best.objective():.2f} | "
                    f"Time: {elapsed_time:.2f}s")
                best.print_metrics()
                
            if stop(self._rng, best, curr):
                break
    ...
    def _determine_outcome(
        ...
        if accept(self._rng, best, curr, cand, self.optimization_direction):  # accept candidate
            outcome = Outcome.ACCEPT

            if self._is_better(cand.objective(), curr.objective()):
                outcome = Outcome.BETTER

        if self._is_better(cand.objective(), best.objective()):  # candidate is new best
            logger.debug(f"New best with objective {cand.objective():.2f}.")
            outcome = Outcome.BEST

        return outcome

    def _is_better(self, new_obj: float, old_obj: float) -> bool:
        """
        Compares two objective values based on optimization direction.
        """
